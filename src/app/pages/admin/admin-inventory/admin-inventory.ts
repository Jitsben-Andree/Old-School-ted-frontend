...
export class AdminInventoryComponent implements OnInit {
  ...
  public inventarioEdit: { [key: number]: number } = {}; // Mapeo ID -> StockNuevo
  ...
  loadInventario(): void {
    ...
    this.inventarioService.getTodoElInventario().subscribe({
      next: (data) => {
        ...
        // Inicializar el objeto de edición con los valores actuales del backend
  ...
  onUpdateStock(item: Inventario): void {
    ...
    if (nuevoStock === null || nuevoStock === undefined || nuevoStock < 0) {
      ...
      this.inventarioEdit[item.inventarioId] = item.stock; // Revertir
      return;
    }

    if (nuevoStock === item.stock) return; // Sin cambios
    ...
    this.inventarioService.actualizarStock(request).subscribe({
      next: (updatedInventory) => {
        ...
        // Feedback opcional (o usar Toast)
        // alert(`Stock actualizado.`);
      },
      error: (err: HttpErrorResponse) => {
        this.error.set(err.message || `Error al actualizar stock.`);
        this.inventarioEdit[item.inventarioId] = item.stock; // Revertir visualmente
        this.status.set('success'); 
